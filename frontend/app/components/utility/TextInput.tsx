...
const TextInput = ({name,id,placeholder,value=undefined,onChange=undefined,style=undefined,className=undefined,errorMsg=undefined}:any) => {
  return (
    ...
    <input
        ...
        className={`${errorMsg ? 'form-input border border-red-600 rounded-md text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:ring-red-500 focus:border-red-500' : '' } block w-full rounded-md border-0 py-1.5 pr-10 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6 ${className}`}
        autoComplete='off'
    />
